...
随着医学影像设备的不断进步和医疗信息化建设的深入，医学影像数据呈现爆炸式增长趋势。肺部 CT 作为肺癌筛查的重要手段，每年产生海量图像数据。然而，传统医学影像分析高度依赖放射科医生的人工阅片，存在效率瓶颈、规模化困难和诊断一致性差异等问题。在这种背景下，人工智能辅助诊断技术应运而生，旨在通过自动化手段提高影像分析效率、增强诊断一致性，为临床医生提供智能支持。

### 专业学习背景

本项目来源于在生物医学信息学、医学图像处理以及人工智能技术交叉学习过程中的实践探索。在系统学习医学数据处理、医学图像分析、深度学习、大语言模型和知识增强技术的基础上，尝试将这些能力整合为一个面向医学场景的智能辅助诊断系统。

### 技术路线来源

传统医学AI系统通常采用单一的"医学影像输入 → 深度学习模型 → 输出检测结果"模式，缺乏知识推理和任务规划能力。Martin 则探索了一条更完整的技术路线：

```
传统医学AI：
医学影像输入 → 深度学习模型 → 输出检测结果

Martin：
医学影像感知能力 + 医学知识增强 + 大模型推理 + Agent智能编排
```

具体而言：
- **MONAI** 负责医学视觉分析，基于 RetinaNet 3D 模型实现肺部结节的精准检测
- **RAG（检索增强生成）** 负责医学知识检索，通过 ChromaDB 和 BGE Embedding 将权威医学指南向量化存储，减少 LLM 幻觉，确保诊断意见有据可查
- **DeepSeek LLM** 负责医学语义推理和报告生成，将检测结果和知识检索内容整合为结构化病例报告
- **LangChain Agent** 负责任务规划和工具调用，根据用户需求智能编排检测、检索和报告生成流程

### 项目价值

Martin 不是简单的检测模型或报告生成工具，而是探索医学 AI 从"单模型能力"向"智能体系统能力"的发展路径。通过整合感知（Vision）、知识（RAG）、推理（LLM）和协同（Agent）四大能力，形成一个面向医学场景的智能辅助诊断系统，旨在为临床医生提供更全面、更可靠的智能支持。
