...
"""
    ...
    def _format_nodules_zh(self, nodules) -> str:
        ...
        for nodule in nodules[:5]:  # 最多显示5个
            lines.append(f"- 结节 {nodule['index']}: 直径 {nodule['diameter']:.2f}mm, 置信度 {nodule['score']:.2%}")
        if len(nodules) > 5:
    ...
    def _format_nodules_en(self, nodules) -> str:
        ...
        for nodule in nodules[:5]:
            lines.append(f"- Nodule {nodule['index']}: {nodule['diameter']:.2f}mm, confidence {nodule['score']:.2%}")
        if len(nodules) > 5:
...
""")
    ...
    def _format_nodules_table_zh(self, nodules) -> str:
        ...
        lines.append("-----|----------|--------|-----|-----|-----")
        for nodule in nodules:
            lines.append(f"{nodule['index']} | {nodule['diameter']:.2f} | {nodule['score']:.4f} | {nodule['center']['x']:.2f} | {nodule['center']['y']:.2f} | {nodule['center']['z']:.2f}")
        return "\n".join(lines)
    ...
    def _generate_impression_zh(self, nodules) -> str:
        ...
        medium_risk = sum(1 for n in nodules if 6 <= n['diameter'] < 8 or 0.8 <= n['score'] <= 0.95)
        ...
        if medium_risk > 0:
            parts.append(f"发现 {medium_risk} 个中等风险结节（直径6-8mm或置信度80-95%）")
        if low_risk > 0:
    ...
    def _generate_impression_en(self, nodules) -> str:
        ...
        if medium_risk > 0:
            parts.append(f"{medium_risk} medium-risk nodule(s) (6-8mm or confidence 80-95%)")
        if low_risk > 0:
            parts.append(f"{low_risk} low-risk nodule(s)")
        
        return ", ".join(parts) + ". Recommend comprehensive evaluation with clinical symptoms and medical history."
    ...
    def generate_with_llm(self, detection_result: Dict, report_type: str = "detailed") -> str:
        """
        ...
        messages = [
            {"role": "system", "content": "你是一位专业的放射科医生，请根据检测结果生成详细的医学报告。"},
